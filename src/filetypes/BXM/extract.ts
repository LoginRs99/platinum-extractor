...
async function extract(file: PlatinumFileReader): Promise<FileData> {
    ...
    function readString(pos: number) {
        ...
        let decoded = enc.decode(arrayBuffer.slice(pos, tmppos));
        if (decoded.includes("")) {
    ...
    function readTree(nodeNum: number): Node {
        ...
        if (dataOffsets[node[3]][0] !== -1 && dataOffsets[node[3]][0] !== 0xFFFF) {
            name = readString(dataOffsets[node[3]][0]);
        }
        if (dataOffsets[node[3]][1] !== -1 && dataOffsets[node[3]][1] !== 0xFFFF) {
        ...
        if (node[2] > 0) {
            for (let i = 0; i < node[2]; i++) {
                let attrname = "";
                let attrvalue = "";
                if (dataOffsets[node[3] + i + 1][0] !== -1 && dataOffsets[node[3] + i + 1][0] !== 0xFFFF) {
                    attrname = readString(dataOffsets[node[3] + i + 1][0]);
                }
                if (dataOffsets[node[3] + i + 1][1] !== -1 && dataOffsets[node[3] + i + 1][1] !== 0xFFFF) {
